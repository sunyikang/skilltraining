...
def conv2d_bn(x,
    ...
    tensor = tf.keras.layers.Conv2D(cv1_out, cv1_filter, strides=cv1_strides,
    ...
    if padding == None:
        return tensor
    tensor = tf.keras.layers.ZeroPadding2D(padding=padding, data_format='channels_first')(tensor)
    if cv2_out == None:
        return tensor
    tensor = tf.keras.layers.Conv2D(cv2_out, cv2_filter, strides=cv2_strides,
